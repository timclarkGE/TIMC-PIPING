...
Added ability to pull in parameters from Ensemble at setup using two fuction calls to open the serial port and pull off
...
class ScanFrame:
    def __init__(self, master, parameters1, parameters2):
        ...
        self.scan_setup = ["0.0", "200.0", "0.0", "100.0", "25.0"]  # Default Values
        self.scan_state = 0
...
class ScanThread(threading.Thread):
    def __init__(self, scan_setup, scan_type, scan_axis, scan_points):
        ...
        self.avg_CIRC_move_time = float()
        self.avg_TRANSLATOR_move_time = float()
        self.i = 0
        self.setDaemon(True)
    ...
    def run(self):
        ...
        while self._is_running:
            ...
            if self.i < (len(self.scan_points)):
                if scan_enabled and scan_in_pos and index_enabled and index_in_pos and not axis_is_moving and not self._is_paused:
                ...
                self.stop()
            """ 
------------ Need something to handle error faults during scan here ------------ 
            """

    def stop(self):
        self._is_running = 0
        self._is_paused = 1
        TIMC.scan.scan_state = 0
        TIMC.acmd("SCAN", "ABORT " + self.scanName)
        TIMC.acmd("SCAN", "ABORT " + self.indexName)
        # Activate Start Scan Button and Axis Buttons
        TIMC.scan.start.config(state="normal", fg="Black")
