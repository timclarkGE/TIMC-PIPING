...
so GUI can start offline if not communicating so this doesn't generate and error when trying to get parameters

Resume scan will analyze location of scanner and determine what action to take:
    if in the scan axis profile while scan axis moving - it will continue scanning
    if out of the scan axis profile while scan axis moving - it will go to last scan point
    if index axis is moved at all - it will go to last scan line (too complex to code based on scan input points)

Write scan setup (scan start/stop, index start/stop, index size) to setup file and recall them when opening

Added menubar to open parameter windows and ability to change counts/unit parameter and reset controller
...
class ToolFrame:
    def __init__(self, master, params):
        ...
        self.toolButton = Button(topFrame, text="NOVA", fg="Black", bg="Deep Sky Blue", font="Helvetica, 14 bold",
                                 command=lambda: self.toggle_tool())

        # Set Button Name and Color based on Tool from Setup File
        if self.toolName == "LPS-1000":
    ...
    def toggle_tool(self):
        if self.toolName == "NOVA" and TIMC.axis1.state == 0 and TIMC.axis2.state == 0:
            self.toolName = "LPS-1000"
            self.toolButton.config(text=self.toolName, fg="Black", bg="Goldenrod")
            TIMC.master.config(bg="Light Goldenrod")
            TIMC.params[0] = "LPS-1000\n"
        ...
        elif self.toolName == "LPS-1000" and TIMC.axis1.state == 0 and TIMC.axis2.state == 0:
            self.toolName = "NOVA"
            self.toolButton.config(text=self.toolName, fg="Black", bg="Deep Sky Blue")
            ...
            messagebox.showinfo("Tool Change Error", "Both axes must be disabled before changing tool")
...
class ScanFrame:
    def __init__(self, master, axis1_parameters, axis2_parameters, scan_parameters):
        ...
        self.scan_type = 2          # Bidirectional
    ...
    def start_scan(self):
        ...
        print(self.scanTypeVar.get(), self.scan_setup)
...
class ScanThread(threading.Thread):
    ...
    def run(self):
        print("Running Scan")
        print("SCAN AXIS: " + TIMC.scan.scanConfigVar.get(), self.scan_points)
        ...
        while self._is_running:
            if TIMC.online == 1:
                ...
                # Option 1 = COMMIT PARAMETERS
                    ...
                    TIMC.fault.update_status("COMMITTING PARAMETERS...")
                    result = find_port()
                    s = serial.Serial(result[0], baud, timeout=0.05)
                ...
                if TIMC.online:
                    ...
                    print("COMPLETE")
                    if self.option == 1:
                        messagebox.showinfo("Parameters", "Reset for Changes to Take Effect")
                    time.sleep(.5)
                    TIMC.fault.fault_ack()
...
class ProgramMenu:
    def __init__(self, master, ws, hs):
        ...
        menubar.add_command(label="Parameters", command=lambda: self.openParameters())
        menubar.add_separator()
        menubar.add_command(label="Exit", command=lambda: on_closing())

    # Open a new window and display parameters that are used in the TIMC Program
    def openParameters(self):
        self.parameter_window = Tk()
        w = 500
        h = 250
        x = self.ws*.5 - (w/2)
        y = (self.hs/2) - (h/2)
        self.parameter_window.geometry("%dx%d+%d+%d" % (w, h, x, y))
        frame = Frame(self.parameter_window)
        frame.pack()
        bottom_frame = Frame(self.parameter_window)
        bottom_frame.pack()
        tool_label = Label(frame, text=TIMC.tool.toolName, width=30, font="Helvetica, 18")
        axis1label = Label(frame, text=TIMC.axis1.axisName, width=12, font="Helvetica, 12 bold")
        axis2label = Label(frame, text=TIMC.axis2.axisName, width=12, font="Helvetica, 12 bold")
        ...
        for i in range(0, 5):
            ...
            self.axis1entry.append(Entry(frame, width=10, font='Helvetica, 10', justify='center'))
            self.axis1entry[i].insert(0, TIMC.axis1.parm_output_list[i])
            self.axis2entry.append(Entry(frame, width=10, font='Helvetica, 10', justify='center'))
        ...
        save_btn = Button(bottom_frame, text="SAVE", width=12, bg="Light Grey", font="Helvetica, 12 bold", command=lambda: self.save_parameters())
        reset_btn = Button(bottom_frame, text="RESET", width=12, bg="Light Grey", font="Helvetica, 12 bold", command=lambda: self.reset_controller())

        save_btn.grid(column=0, row=0, padx=30, pady=15)
        reset_btn.grid(column=1, row=0, padx=30, pady=15)

    # This will save and commit new parameters
    def save_parameters(self):
        print("SAVE AND COMMIT")
        new_param = 0
        # Get Entry Values
        ...
        for i in range(0, 5):
            ...
            if TIMC.axis1.parm_output_list[i] != axis1_new_parm_output_list[i]:
                new_param = 1
                TIMC.acmd(self.queue, "SETPARM " + TIMC.axis1.axisName + ", " + parm_num_list[i] + ", " + axis1_new_parm_output_list[i])
            ...
            if TIMC.axis2.parm_output_list[i] != axis2_new_parm_output_list[i]:
                new_param = 1
                TIMC.acmd(self.queue, "SETPARM " + TIMC.axis2.axisName + ", " + parm_num_list[i] + ", " + axis2_new_parm_output_list[i])
...
class Main:
    def __init__(self, master):
        ...
        # Create Menu Heading
        self.menu = ProgramMenu(self.master, self.ws, self.hs)
...
def getparams(axisName):
    ...
    if port_open == 0:
        ...
        parm_output_list = [2, 5, 25, "mm", 250, 2]
...
def on_closing():
    ...
    exception_flag = 0
    try:
        TIMC.menu.parameter_window.destroy()
    except:
        pass
