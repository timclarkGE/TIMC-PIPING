...
##############################################
# Revision Updates:
#   <> Fault status was called for each axis separated by 0.5 seconds, now I call both simultaneously and
#      then wait 1 second
#   <> When checking for a fault, I no longer check if the axis state is enabled. Lines: 1029 and 1037
#   <> Removed estop flag
#   <> Updated logic related to start, stop, and index size. Previously I used modulo on non integers which
#      was causing errors. Line: 622
#   <> Updated logic to include a check that both axes are enabled at the start of a scan. Without this the
#      elapsed time was getting messed up. Line: 635
