    ...
    def run(self):
        while self._is_running:
            if TIMC.online:
                ...
                TIMC.menu.deactivate_prgm_btns()
                TIMC.axis1.enableButton.config(state="disabled")
                TIMC.axis2.enableButton.config(state="disabled")
...
class ProgramMenu:
    ...
    def openParameters(self):
        ...
        self.save_btn = Button(self.bottom_frame, text="SAVE AND COMMIT", width=15, bg="Light Blue", font="Helvetica, 10 bold",
    ...
    def save_parameters(self):
        ...
        # If values have changed and TIMC is online, use SETPARM to save the new value
        ...
        # Reverse Motor Directions for Reverse Positive Motion parameter
        ...
        if new_param == 1:
            self.process_reset = ResetThread(1)  # Initiate the Reset Thread to stop communication and Save Parameters
    ...
    def reset_controller(self):
        print("RESET")
        if TIMC.online:
            self.process_reset = ResetThread(0)  # Initiate the Reset Thread to stop communication and Reset Controller
            self.process_reset.start()

    def activate_prgm_btns(self):
        self.save_btn.config(state="normal", fg="Black", bg="Light Blue")
        self.reset_btn.config(state="normal", fg="Black", bg="Light Green")

    def deactivate_prgm_btns(self):
