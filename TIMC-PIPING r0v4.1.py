    ...
    def toggle_tool(self):
        ...
        if not TIMC.online:
            if self.toolName == "NOVA":
                # Switch to LPS-1000, Update Setup File
            ...
            messagebox.showinfo("Tool Change Button", "Cannot change tool now\n" "Change parameter file to change tool")
...
class AxisFrame:
    def __init__(self, master, parameters):
        ...
        self.position_box = Label(self.pos_frame, text=str(round(self.position, decpl)), fg="Gray", bg="White", width=7, anchor=E, font="Helvetica, 26 bold")
        self.velocity_box = Label(self.pos_frame, text=str(round(self.velocity, decpl)), bg="White", width=8, state="disabled", font=("Helvetica", 8))
    ...
    def setTo(self, zero):
        ...
        if check_is_digit(position):
            if TIMC.online == 1:
                ...
                self.position_box.config(text=str(round(self.position, decpl)))
        ...
        while self._is_running:
            ...
            if TIMC.qFBK_read.qsize() and TIMC.online:
                ...
                if self.read_index == 0:
                    ...
                    TIMC.axis1.position_box.config(text=str(round(pos, decpl)))
                    self.read_index += 1
                    ...
                    TIMC.axis1.velocity_box.config(text=str(round(vel, decpl)))
                    self.read_index += 1
                    ...
                    pos = round(float(data), 5)
                    TIMC.axis2.position_box.config(text=str(round(pos, decpl)))
                    self.read_index += 1
                    ...
                    vel = round(float(data), 5)
                    TIMC.axis2.velocity_box.config(text=str(round(vel, decpl)))
                    self.read_index += 1
...
class ResetThread(threading.Thread):
    ...
    def run(self):
        ...
        # Disable Program Menu Buttons in case user closes window during reset
        TIMC.menu.deactivate_param_btns()
        TIMC.axis1.enableButton.config(state="disabled")
        TIMC.axis2.enableButton.config(state="disabled")

        # Processes Stopped Flag
        self.process_stop = 1
        ...
        while self._is_running:
            ...
            while self.process_stop:
                s = serial.Serial(result[0], baud, timeout=0.05)
                # Send a command to check if communication has been established
                s.write("ACKNOWLEDGEALL".encode('ascii') + b' \n')
                data = s.readline().decode('ascii')
                if '%' in data:
                    # Turn process stop flag off
                    self.process_stop = 0
                s.close()

            # If user selected Commit and Reset, go back through and Reset
            # Turn process stop flag ON and change option to 0 for _is_running loop
            if self.option == 1:
                self.process_stop = 1
                self.option = 0
            # Else restart all processes again
            else:
                TIMC.start_serial()
                # If the reason for the reset was to change the axis direction, use the position saved prior to reset
                # Change the sign of that value so operator does not lose zero position
                if TIMC.online:
                ...
                # Update all axis entry boxes with values from parameter file in case operator changed value
                # but did not save and commit and just clicked "RESET"
                if TIMC.menu.open:
                    for i in range(0, 5):
    ...
    def __init__(self, master):
        ...
        if self.online:
            if self.axis1.motorType == 2:
                self.params[0] = "LPS-1000\n"
                self.tool.toolName = "LPS-1000"
                self.axis1.motorMultiplier = -1     # LPS-1000 Translator direction is reversed
                self.tool.toolButton.config(text="LPS-1000", bg="Goldenrod")
                master.configure(background="Light Goldenrod")
