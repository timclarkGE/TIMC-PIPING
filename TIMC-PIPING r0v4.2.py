...
class ScanFrame:
    ...
    def start_scan(self):
        self.scan_state = 1     # Scan State Flag
        TIMC.fault.fault_ack()  # Clear Fault Status
...
class ScanThread(threading.Thread):
    ...
    def calc_scan_time(self, start_i, start_j):
        ...
        if self.scan_axis == "axis1":
            scan_position = round(float(TIMC.axis1.position.get()), 2)
            index_position = round(float(TIMC.axis2.position.get()), 2)
        else:
            scan_position = round(float(TIMC.axis2.position.get()), 2)
            index_position = round(float(TIMC.axis1.position.get()), 2)
