...
class ProgramMenu:
    ...
    def openParameters(self):
        ...
        # If LAPIS, Checkbox is same logic as parameter file, if NOVA, flip logic value
        if TIMC.axis1.motorType == 2:
            self.axis1_dir.set(int(TIMC.axis1.parm_output_list[6]))
        else:
            self.axis1_dir.set(not int(TIMC.axis1.parm_output_list[6]))
        self.axis2_dir_box = Checkbutton(self.parameter_frame, variable=self.axis2_dir)
        self.axis2_dir_box.grid(column=2, row=12)
        # If LAPIS, Checkbox is same logic as parameter file, if NOVA, flip logic value
        if TIMC.axis2.motorType == 2:
            self.axis2_dir.set(int(TIMC.axis2.parm_output_list[6]))
        else:
            self.axis2_dir.set(not int(TIMC.axis2.parm_output_list[6]))
    ...
    def save_parameters(self):
        ...
        # Check for change in Reverse Positive Motion parameter for LAPIS (Checkbox logic SAME as parameter file)
        if TIMC.tool.toolName == "LAPIS":
            if self.axis1_dir.get() != int(TIMC.axis1.parm_output_list[6]):
                new_param = 1
                self.axis1_dir_change = 1   # Flags the reset thread to save the current position and switch sign
                if TIMC.online:
            ...
            if self.axis2_dir.get() != int(TIMC.axis2.parm_output_list[6]):
                new_param = 1
                self.axis2_dir_change = 1   # Flags the reset thread to save the current position and switch sign
                if TIMC.online:
        ...
        # Check for change in Reverse Positive Motion parameter for NOVA (Checkbox logic REVERSE as parameter file)
        else:
            if self.axis1_dir.get() == int(TIMC.axis1.parm_output_list[6]):
                new_param = 1
                self.axis1_dir_change = 1  # Flags the reset thread to save the current position and switch sign
                # Use a mirror variable for NOVA to set parameter and direction
                if self.axis1_dir.get():
                    axis1_dir = 0
                else:
                    axis1_dir = 1
                if TIMC.online:
                    TIMC.acmd(self.queue, "SETPARM " + TIMC.axis1.axisName + ", " + parm_num_list[6] + ", " + str(
                        axis1_dir))
                    TIMC.axis1.motorDirection = axis1_dir
                    TIMC.axis1.parm_output_list[6] = str(axis1_dir)

            if self.axis2_dir.get() == int(TIMC.axis2.parm_output_list[6]):
                new_param = 1
                self.axis2_dir_change = 1  # Flags the reset thread to save the current position and switch sign
                # Use a mirror variable for NOVA to set parameter and direction
                if self.axis2_dir.get():
                    axis2_dir = 0
                else:
                    axis2_dir = 1
                if TIMC.online:
                    TIMC.acmd(self.queue, "SETPARM " + TIMC.axis2.axisName + ", " + parm_num_list[6] + ", " + str(
                        axis2_dir))
                    TIMC.axis2.motorDirection = axis2_dir
                    TIMC.axis2.parm_output_list[6] = str(axis2_dir)
    ...
    def __init__(self, master):
        ...
        if self.online:
            ...
            if self.axis1.motorType == 3:
                ...
                self.axis2.motorMultiplier = -1     # NOVA Circ direction is reversed
